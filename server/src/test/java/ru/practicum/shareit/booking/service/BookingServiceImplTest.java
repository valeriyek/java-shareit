...
class BookingServiceImplTest {
    ...
    void initialize() {
        ...
        // Создаём пользователя (booker)
        booker = userService.save(
                new UserDto(
                        null,
                        "Mike",
                        "mike@mail.ru")
        ...
        // Создаём DTO для сохранения бронирования
        BookingSavingDto bookingSavingDto = BookingSavingDto.builder()
                .start(now().plusDays(1)) // Начало бронирования завтра
                .end(now().plusDays(2))   // Конец бронирования послезавтра
    ...
    @Test
    void getBookingsByItemTest() {
        var bookingsFromService = bookingService.getBookingsByItem(
                itemDto.getId(),
                owner.getId()
        );

        var bookingsFromDb = entityManager.createQuery(
                        "SELECT booking FROM Booking booking " +
                                "JOIN booking.item item " +
                                "WHERE item.owner.id = :ownerId AND item.id = :itemId",
                        Booking.class)
        ...
        assertThat(bookingsFromService.size(), equalTo(bookingsFromDb.size()));
        if (!bookingsFromDb.isEmpty()) {
            assertThat(bookingsFromService.get(0).getId(), equalTo(bookingsFromDb.get(0).getId()));
        }
    }
